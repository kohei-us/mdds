...
void mtv_test_position()
{
    ...
    typename mtv_type::iterator check;
    typename mtv_type::const_iterator const_check;
    db.set(6, 1.1);
    db.set(7, 1.2);
    db.set(8, 1.3);
    db.set(9, 1.4);

    typename mtv_type::position_type pos = db.position(0);
...

    {
        ...
        typename mtv_type::iterator it = pos.first;
    ...
    typename mtv_type::const_position_type const_pos = db_ref.position(3);
...
void mtv_test_position_next()
{
    ...
    TEST_ASSERT(mtv_type::template get<mdds::mtv::double_element_block>(pos) == 1.1);

    pos = mtv_type::next_position(pos);
    TEST_ASSERT(mtv_type::logical_position(pos) == 3);
    TEST_ASSERT(pos.first->type == mdds::mtv::element_type_double);
    TEST_ASSERT(mtv_type::template get<mdds::mtv::double_element_block>(pos) == 1.2);

    pos = mtv_type::next_position(pos);
    TEST_ASSERT(mtv_type::logical_position(pos) == 4);
    TEST_ASSERT(pos.first->type == mdds::mtv::element_type_string);
    TEST_ASSERT(mtv_type::template get<mdds::mtv::string_element_block>(pos) == "A");

    pos = mtv_type::next_position(pos);
    TEST_ASSERT(mtv_type::logical_position(pos) == 5);
    TEST_ASSERT(pos.first->type == mdds::mtv::element_type_string);
    TEST_ASSERT(mtv_type::template get<mdds::mtv::string_element_block>(pos) == "B");

    pos = mtv_type::next_position(pos);
    TEST_ASSERT(mtv_type::logical_position(pos) == 6);
    TEST_ASSERT(pos.first->type == mdds::mtv::element_type_string);
    TEST_ASSERT(mtv_type::template get<mdds::mtv::string_element_block>(pos) == "C");

    pos = mtv_type::next_position(pos);
    TEST_ASSERT(mtv_type::logical_position(pos) == 7);
    TEST_ASSERT(pos.first->type == mdds::mtv::element_type_boolean);
    TEST_ASSERT(mtv_type::template get<mdds::mtv::boolean_element_block>(pos) == true);

    pos = mtv_type::next_position(pos);
    TEST_ASSERT(mtv_type::logical_position(pos) == 8);
    TEST_ASSERT(pos.first->type == mdds::mtv::element_type_boolean);
    TEST_ASSERT(mtv_type::template get<mdds::mtv::boolean_element_block>(pos) == false);

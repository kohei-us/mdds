...
void mtv_test_erase()
{
...

    {
        ...
        mtv_type db(5);
        db.template push_back<int32_t>(-234);
...

    {
        ...
        TEST_ASSERT(db.template get<uint64_t>(2) == 10);
        TEST_ASSERT(db.is_empty(3));
...

    {
        ...
        TEST_ASSERT(db.template get<double>(0) == 1.0);
        TEST_ASSERT(db.template get<double>(1) == 2.0);
        TEST_ASSERT(db.template get<std::string>(2) == "A");
        TEST_ASSERT(db.template get<std::string>(3) == "B");
        TEST_ASSERT(db.template get<double>(4) == 5.0);
        TEST_ASSERT(db.template get<double>(5) == 6.0);
        db.erase(1, 4);
        TEST_ASSERT(db.block_size() == 1);
        TEST_ASSERT(db.size() == 2);
        TEST_ASSERT(db.template get<double>(0) == 1.0);
        TEST_ASSERT(db.template get<double>(1) == 6.0);
    }

    {
        mtv_type db(6);
        db.set(0, 1.0);
        db.template set<int8_t>(4, 2);
        db.template set<int16_t>(5, 3);
        db.erase(1, 3);
    }

    {
        ...
        for (int i = 0; i < 3; ++i)
            db.template push_back<int16_t>(3);

        for (int i = 0; i < 3; ++i)
            db.template push_back<int32_t>(9);
...

    {
        ...
        for (int i = 0; i < 3; ++i)
            db.template push_back<double>(9);
...

    {
        ...
        for (int i = 0; i < 3; ++i)
            db.template push_back<double>(1.23);

...
void mtv_test_iterators_set()
{
    ...
    // single element only
    mtv_type db(1);
    typename mtv_type::iterator it = db.set(0, 1.1);
    ...
    TEST_ASSERT(it == db.begin());
    TEST_ASSERT(it->size == 4);
    TEST_ASSERT(it->type == mdds::mtv::element_type_uint16);
    std::advance(it, 3);
    TEST_ASSERT(it == db.end());
...
void mtv_test_iterators_set_2()
{
    ...
    typename mtv_type::iterator it, check;
    std::vector<double> doubles(3, 1.1);
    ...
    TEST_ASSERT(db.block_size() == 6);
    doubles.resize(4, 4.5);
    it = db.set(5, doubles.begin(), doubles.end()); // 5 thrugh 8.
    check = db.begin();
    TEST_ASSERT(check->type == mdds::mtv::element_type_int32);
    ++check;
    ...
    check = db.begin();
    TEST_ASSERT(check->type == mdds::mtv::element_type_boolean);
    TEST_ASSERT(check->size == 5); // 0 thru 4
    ++check;
    TEST_ASSERT(check->type == mdds::mtv::element_type_double);
    TEST_ASSERT(check->size == 2); // 5 thru 6
    ++check;
    TEST_ASSERT(it == check);
    ++it;
    TEST_ASSERT(it->type == mdds::mtv::element_type_boolean);
    TEST_ASSERT(it->size == 4); // 16 thru 19
    ++it;
    TEST_ASSERT(it == db.end());
}

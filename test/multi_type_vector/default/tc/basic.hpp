...
void mtv_test_basic()
{
...

    {
        ...
        TEST_ASSERT(db.template get<double>(1) == 1.0);
        TEST_ASSERT(db.template get<double>(2) == 2.0);
        TEST_ASSERT(db.template get<std::string>(3) == "foo");
        TEST_ASSERT(db.template get<double>(4) == 3.0);
        TEST_ASSERT(db.template get<double>(5) == 4.0);
        ...
        TEST_ASSERT(db.template get<double>(1) == 1.0);
        TEST_ASSERT(db.template get<double>(2) == 2.0);
        TEST_ASSERT(db.template get<double>(3) == 5.0);
        TEST_ASSERT(db.template get<double>(4) == 3.0);
        TEST_ASSERT(db.template get<double>(5) == 4.0);
    }

    {
        ...
        TEST_ASSERT(db.template get<std::string>(5) == "test");
...

    {
        ...
        TEST_ASSERT(db.template get<double>(7) == 2.1);
    }

    {
        ...
        TEST_ASSERT(db.template get<double>(3) == 4.1);
        TEST_ASSERT(db.template get<double>(4) == 1.2);
        TEST_ASSERT(db.template get<double>(5) == 1.2);
        TEST_ASSERT(db.template get<double>(6) == 1.2);
        TEST_ASSERT(db.template get<bool>(7) == true);
    }

    {
        mtv_type db(10, false);
        db.template set<int8_t>(0, 'a');
        db.template set<int8_t>(1, 'b');
        db.template set<int8_t>(2, 'c');

        db.template set<uint8_t>(3, 'd');
        db.template set<uint8_t>(4, 'e');
        db.template set<uint8_t>(5, 'f');

        TEST_ASSERT(db.block_size() == 3);
        db.template set<int8_t>(0, 'r'); // overwrite.
        db.template set<uint8_t>(5, 'z'); // overwrite

        TEST_ASSERT(db.block_size() == 3);
        typename mtv_type::const_iterator it = db.begin();
...
void mtv_test_basic_numeric()
{
    ...
    db.template push_back<double>(0.0);
    db.template push_back<double>(1.0);
    db.template push_back<double>(2.0);

    TEST_ASSERT(db.size() == 3);
    TEST_ASSERT(db.block_size() == 1);

    db.template set<float>(1, 4.0f);
    TEST_ASSERT(db.size() == 3);
    TEST_ASSERT(db.block_size() == 3);

    db.template set<float>(0, 3.5f);
    TEST_ASSERT(db.size() == 3);
    TEST_ASSERT(db.block_size() == 2);

    db.template set<float>(2, 4.5f);
    TEST_ASSERT(db.size() == 3);
    TEST_ASSERT(db.block_size() == 1);
}

...
void mtv_test_iterators()
{
    ...
    {
        mtv_type db(5);
        typename mtv_type::const_iterator it;
        it = db.begin();
        typename mtv_type::const_iterator it_end = db.end();
        ...
        const typename mtv_type::const_iterator::value_type& val = *it;
...

    {
        ...
        {
            // Forward iterator
            typename mtv_type::const_iterator it = db.begin(), it_end = db.end();
            size_t len = std::distance(it, it_end);
            TEST_ASSERT(len == 3);
            TEST_ASSERT(it != it_end);
        ...
        {
            // Reverse iterator
            typename mtv_type::const_reverse_iterator it = db.rbegin(), it_end = db.rend();
            size_t len = std::distance(it, it_end);
            TEST_ASSERT(len == 3);
            TEST_ASSERT(it != it_end);
            TEST_ASSERT(it->type == mdds::mtv::element_type_string);
...

        {
            ...
            typename mtv_type::const_reverse_iterator it = db.crbegin(), it_end = db.crend();
            size_t len = std::distance(it, it_end);
            TEST_ASSERT(len == 3);
            TEST_ASSERT(it != it_end);
            TEST_ASSERT(it->type == mdds::mtv::element_type_string);
...
void mtv_test_iterators_element_block()
{
    ...
    typename mtv_type::const_iterator it_blk = db.begin(), it_blk_end = db.end();
...
void mtv_test_iterators_mutable_element_block()
{
     ...
     */
    auto check_block_iterator = [](const typename mtv_type::iterator& it, mdds::mtv::element_t expected) {
    ...
    mtv_type db(1);
    db.set(0, 1.2);
    typename mtv_type::iterator it_blk = db.begin(), it_blk_end = db.end();
    ...
    *it = 2.3; // write via iterator.
    TEST_ASSERT(db.template get<double>(0) == 2.3);
    ...
    *it = 3.3;
    ...
    TEST_ASSERT(db.template get<double>(0) == 3.1);
    TEST_ASSERT(db.template get<double>(1) == 3.2);
    TEST_ASSERT(db.template get<double>(2) == 3.3);
}
...
void mtv_test_iterators_private_data()
{
    ...
    typename mtv_type::iterator it = db.begin();
    TEST_ASSERT(it->position == 0);

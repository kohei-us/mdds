...
void mtv_test_misc_value_type()
{
    ...
    std::for_each(db.begin(), db.end(), [](const typename mtv_type::value_type& node) {
...
void mtv_test_misc_push_back()
{
    ...
    typename mtv_type::iterator it = db.push_back_empty();
    ...
    TEST_ASSERT(db.template get<double>(2) == 1.1);
    ...
    ++it;
    TEST_ASSERT(it == db.end());
    TEST_ASSERT(db.template get<std::string>(0) == "push me");
    ...
    TEST_ASSERT(db.template get<std::string>(0) == "push me");
    TEST_ASSERT(db.template get<std::string>(1) == "again");
